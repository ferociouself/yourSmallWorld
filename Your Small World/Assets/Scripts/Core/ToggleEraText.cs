...
public class ToggleEraText : MonoBehaviour {
	...
	public void ChangeEraText(int curTier){
		switch (curTier) {
		case(0):
			this.gameObject.GetComponent<Text>().text = "Prologue - The Awakening".ToUpper();
			info.transform.GetChild(0).gameObject.GetComponent<Text>().text = "Hear your name and awaken great one! The world is desolate and harsh, not yet ready for life. Carve out a valley and fill it with water, then plant forests along the banks.".ToUpper();
			break;
		case(1):
			this.gameObject.GetComponent<Text>().text = "The Age of Stone".ToUpper();
			info.transform.GetChild(0).gameObject.GetComponent<Text>().text = "Congratulations, your newly arisen people have built themselves a home, taking the first step on the road of civilization. But there is much work to do yet, and many obstacles to overcome. In order to expand, your people need simple huts to live in, a workshop to make primitive stone tools, and a well for a more stable source of water.".ToUpper();
			break;
		case(2):
			this.gameObject.GetComponent<Text> ().text = "The Age of Iron".ToUpper();
